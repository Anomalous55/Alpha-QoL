# Slightly-Better-Alpha

***Content to be added soon!***

This is a .jar mod for Minecraft Alpha 1.1.2_01 that only slightly changes the game for a better vanilla experience!

## Disclaimer
I am not affiliated with Mojang or Microsoft. The intention of this mod is purely entertainment. This repository does NOT intend to illegally distribute or promote the distribution of the Minecraft source code owned by Microsoft. Should the removal of this source code be requested by a Mojang or Microsoft representative, action will be taken immediately.
***Absolutely no copyright infringement is intended by the hosting of this mod on GitHub.***

## About
Minecraft Alpha 1.1.2_01 has many glitches/bug that make playing this brutal version much harder, annoyingly so. That wasn't going to stop me, though.
Other mods released, while incredibly technically impressive and fix the same things that this mod does, add just a little too much for my liking, so I decided to make a pretty minimal mod that only changes the game in a very slight quality of life way without any real additions. 

## Changes
Version 1.0
* Armor's Defensive value is no longer tied to Durability
* Doubled the durability of all tools.
* Pickaxes and Axes now properly break the blocks they should break.
* Porkchops and Cooked Porkchops Stack up to 8
* Snow on Winter Mode worlds is now less obtrusive.

## Installing

TBA
